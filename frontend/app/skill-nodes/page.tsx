...
export default function SkillNodesPage() {
  ...
  const loadSkillNodes = async () => {
    try {
      setLoading(true);
      const response = await apiClient.getSkillNodes(page, 20);
      setSkillNodes(response.data);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiClient.deleteSkillNode(id);
      loadSkillNodes();
    } catch (err) {
  ...
  const handleSubmit = async (data: Partial<SkillNode>) => {
    try {
      if (editing) {
        await apiClient.updateSkillNode(editing.id, data);
      } else {
        await apiClient.createSkillNode(data);
      }
      setShowForm(false);
      setEditing(null);
      loadSkillNodes();
    ...
    <DashboardLayout>
      <div className="px-4 py-6 sm:px-0">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Skill Nodes</h1>
          <button
        ...
        {showForm && (
          <div className="mb-6 p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
            <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">
              {editing ? 'Edit Skill Node' : 'Create Skill Node'}
            </h2>
            <EntityForm
              entity={editing}
              type="skill-node"
