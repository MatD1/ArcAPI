...
export default function AppwritePage() {
  ...
  useEffect(() => {
    // Always check if Appwrite is enabled, even when not authenticated
    checkEnabled();
    ...
    if (oauthStatus === 'success') {
      ...
      // Refresh user session and reload data
      setTimeout(async () => {
        try {
          const session = await getAppwriteSession();
          if (session) {
            setAppwriteUser(session);
            // Reload counts to reflect authenticated access
            await loadCounts();
          }
        } catch (err) {
  ...
  const checkEnabled = async () => {
    const enabled = await isAppwriteEnabled();
    setEnabled(enabled);
  };
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      // loginWithGitHub will redirect, so we don't need to do anything else
    } catch (err) {
      setError(getErrorMessage(err));
      setAppwriteAuthLoading(false);
    }
  ...
  const handleDiscordLogin = async () => {
    ...
    try {
      ...
      // loginWithDiscord will redirect, so we don't need to do anything else
    } catch (err) {
      setError(getErrorMessage(err));
      setAppwriteAuthLoading(false);
    }
  ...
  const handleAPIKeyLogin = async (e: React.FormEvent) => {
    ...
    try {
      await login(apiKey);
      // After successful login, reload the page data
      await checkEnabled();
      await loadCounts();
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  const handleForceSync = async () => {
    ...
    if (!confirm('This will sync all data from the API to Appwrite. This may take a while. Continue?')) {
      return;
    }
        ...
        <div className="max-w-md w-full space-y-8 p-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
          <div>
            ...
            <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
              Sign in to access Appwrite Management
            </p>
          </div>

          {/* Backend API OAuth Buttons */}
          <div>
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Backend API OAuth</h3>
            <div className="space-y-3">
              ...
              >
                ...
                Sign in with GitHub (Backend API)
              </button>
              ...
              >
                ...
                Sign in with Discord (Backend API)
              </button>
            </div>
          </div>

          {/* Appwrite OAuth Buttons */}
          <>
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400">
                  Or use Appwrite OAuth
                </span>
              </div>
            </div>
            <div>
              <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Appwrite OAuth</h3>
              {!enabled && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                  Appwrite is still initializing. Please wait or verify your Appwrite configuration.
                </p>
              )}
              <div className="space-y-3">
                {/* Appwrite GitHub OAuth Button */}
                <button
                  type="button"
                  onClick={handleGitHubLogin}
                  disabled={appwriteAuthLoading || !enabled}
                  className="w-full flex items-center justify-center px-4 py-3 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-gray-800 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                    <path
                  ...
                  {appwriteAuthLoading ? 'Connecting...' : 'Sign in with GitHub (Appwrite)'}
                </button>

                {/* Appwrite Discord OAuth Button */}
                <button
                  type="button"
                ...
                >
                  ...
                  {appwriteAuthLoading ? 'Connecting...' : 'Sign in with Discord (Appwrite)'}
                </button>
              </div>
            </div>
          </>
          ...
          <form className="space-y-6" onSubmit={handleAPIKeyLogin}>
            <div>
  ...
  return (
    <DashboardLayout>
      <div className="px-4 py-6 sm:px-0">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Appwrite Management</h1>
        ...
        {enabled && (
          <div className="mb-6 p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Appwrite Access Control</h2>
            ...
            <div className="mt-4">
              ...
              <span className="ml-2 text-gray-900 dark:text-white">
                {appwriteUser ? `Signed in as ${appwriteUser.email || appwriteUser.name || appwriteUser.$id}` : 'Not signed in'}
              </span>
            </div>
            <div className="mt-4 space-y-4">
              {appwriteUser ? (
                ...
                >
                  {appwriteAuthLoading ? 'Signing out...' : 'Sign out of Appwrite'}
                </button>
              ) : (
                <>
                  {/* Appwrite OAuth */}
                  <div>
                    <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Appwrite OAuth (Direct)</h3>
                  ...
                  {/* Old System OAuth */}
                  <div>
                    <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Backend API OAuth (Legacy)</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                      Use the backend API OAuth system to authenticate with the main API
                    </p>
                    <div className="flex flex-wrap gap-3">
                      <button
                        onClick={handleOldSystemGitHubLogin}
                        disabled={oauthLoading}
                        className="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-white rounded-md text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                      >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        {oauthLoading ? 'Connecting...' : 'Sign in with GitHub (Backend API)'}
                      </button>
                      <button
                        onClick={handleOldSystemDiscordLogin}
                        disabled={oauthLoading}
                        className="px-4 py-2 bg-[#5865F2] hover:bg-[#4752C4] text-white rounded-md text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                      >
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        {oauthLoading ? 'Connecting...' : 'Sign in with Discord (Backend API)'}
                      </button>
                    </div>
                  </div>
                </>
        ...
        {!enabled && (
          <div className="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
            <p className="text-yellow-800 dark:text-yellow-200 mb-2">
              Appwrite is not enabled. Set <code className="bg-yellow-100 dark:bg-yellow-900 px-1 rounded">NEXT_PUBLIC_APPWRITE_ENABLED=true</code> in your environment variables.
            </p>
            <details className="mt-2">
              ...
              <div className="mt-2 p-2 bg-yellow-100 dark:bg-yellow-900 rounded text-xs font-mono">
                <div>NEXT_PUBLIC_APPWRITE_ENABLED: {process.env.NEXT_PUBLIC_APPWRITE_ENABLED || '(not set)'}</div>
                <div>NEXT_PUBLIC_APPWRITE_ENDPOINT: {process.env.NEXT_PUBLIC_APPWRITE_ENDPOINT ? '✓ Set' : '✗ Not set'}</div>
                <div>NEXT_PUBLIC_APPWRITE_PROJECT_ID: {process.env.NEXT_PUBLIC_APPWRITE_PROJECT_ID ? '✓ Set' : '✗ Not set'}</div>
        ...
        <div className="mb-6 p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Sync all data from the API to Appwrite. This will update or insert all entities.
          </p>
          <button
        ...
        {enabled && (
          <div className="mb-6 p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Data in Appwrite</h2>
              <button
                onClick={loadCounts}
                disabled={loading}
