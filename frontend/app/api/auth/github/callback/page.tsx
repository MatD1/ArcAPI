...
export default function GitHubCallbackPage() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        // For OAuth flow, backend redirects here with a temp token in query string
        // The backend already handled the GitHub OAuth callback

        // Get temp token from URL (backend redirected here with token in query)
        const tempToken = searchParams.get('token');
        
        if (!tempToken) {
          // If no temp token, this might be a direct callback with code (shouldn't happen)
