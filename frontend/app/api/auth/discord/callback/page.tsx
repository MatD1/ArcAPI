...
function getAPIURL() {
  if (process.env.NEXT_PUBLIC_API_URL) {
    console.debug('[Discord Callback] Using NEXT_PUBLIC_API_URL:', process.env.NEXT_PUBLIC_API_URL);
    return process.env.NEXT_PUBLIC_API_URL;
  }
  
  if (typeof window !== 'undefined') {
    console.debug('[Discord Callback] Using window.location.origin:', window.location.origin);
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        if (!tempToken) {
          console.error('[Discord Callback] No token in URL search params');
          setError('No authentication token found. Please try logging in again.');
          setLoading(false);
          return;
        }

        console.debug('[Discord Callback] Token found, exchanging for auth data...');
        
        // Exchange temp token for actual auth data
        const apiURL = getAPIURL();
        const exchangeURL = `${apiURL}/api/v1/auth/exchange-token?token=${tempToken}`;
        
        console.debug('[Discord Callback] Exchange URL:', exchangeURL);
        
        const response = await fetch(exchangeURL);
        
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({ error: 'Authentication failed' }));
          console.error('[Discord Callback] Exchange token failed:', response.status, errorData);
        ...
        console.debug('[Discord Callback] Exchange successful, user:', user?.email);
        ...
        console.debug('[Discord Callback] Auth setup complete, redirecting to dashboard');
        
        // Redirect to dashboard (user is already authenticated via setUser above)
        router.push('/dashboard/');
      } catch (err) {
        const errorMsg = getErrorMessage(err);
        console.error('[Discord Callback] Authentication error:', errorMsg);
        setError(errorMsg);
        setLoading(false);
      }
    };
